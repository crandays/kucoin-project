    ...
    if args.command == "backtest":
        ...
        if args.params:
            if args.params.startswith('@'):
                ...
                params_file = args.params[1:]  # Remove @ prefix
                try:
                    with open(params_file) as f:
                        param_ranges = json.load(f)
                    logger.info(f"Loaded parameter ranges from file: {params_file}")
                except FileNotFoundError:
                    logger.error(f"Parameter file not found: {params_file}")
                    sys.exit(1)
                except json.JSONDecodeError:
                    logger.error(f"Invalid JSON in parameter file: {params_file}")
                    sys.exit(1)
            else:
                ...
                try:
                    ...
                    logger.error("Invalid JSON parameter string")
                    sys.exit(1)
        else:
